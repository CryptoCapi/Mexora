...
const CryptoBackground: React.FC<CryptoBackgroundProps> = ({ type }) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Crear gradiente de fondo según el tipo
      const gradient = ctx.createLinearGradient(0, 0, canvas.width, canvas.height);
      if (type === 'trading') {
        gradient.addColorStop(0, 'rgba(0, 0, 0, 0.1)');
        gradient.addColorStop(1, 'rgba(255, 215, 0, 0.1)'); // Dorado
      } else {
        gradient.addColorStop(0, 'rgba(0, 0, 0, 0.1)');
        gradient.addColorStop(1, 'rgba(63, 81, 181, 0.1)'); // Azul
      }
